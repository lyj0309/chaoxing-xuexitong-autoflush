...
class TestList {
  ...
  async getDetail() {
        ...
        switch (type) {
          case 1: {
            ...
            if (hasdone) {
              ...
              for (let i = 0; i < answers.length; i++) {
                ...
                if (key.indexOf("我的答案") != -1) built.answer = value;
              }
              built.correct = iscorrect;
            } else {
              let options = ques.find(".Zy_ulTop").find("li");

              built.optionid = options
                .eq(0)
                .find("input")
                .eq(0)
                .attr("name")
          ...
          case 2: {
            ...
            for (let i = 0; i < answerkeys.length; i++) {
              if (answerkeys.eq(i).find(".fr").length == 0) continue;

              let answer = answerkeys.eq(i).find(".clearfix");
              let content = answer.text();

              let iscorrect =
                answerbar.find(".fr").attr("class").indexOf("dui") != -1;
            ...
            if (hasdone) {
              ...
              for (let m = 0; m < built.answers.length; m++)
                if (!built.answers[m].correct) correct = false;
              ...
              built.optionid = JSON.stringify({
                id: lastinput.attr("name").replace("tiankongsize", ""),
                subids: optionids,
              });
          ...
          case 3: {
            ...
            if (hasdone) {
              ...
              built.answer = false;
              if (answerstr == "√") built.answer = true;
    ...
    let params = {};
    if (status == 0) params = await this.getParams($);
  ...
  getStatus($) {
    //判断状态：0为待做，1为待批阅 ,2为已完成
    let top = $(".ZyTop").find("h3");
    let statustext = top.find("span").eq(0).text();

    if (statustext.indexOf("待做") != -1) return 0;
    else if (statustext.indexOf("待批阅") != -1) return 1;
    else if (statustext.indexOf("已完成") != -1) return 2;
    else return -1;
  }
  ...
  getType(ques, title, status) {
    ...
    else if (title.indexOf("【简答题】") != -1) return 4;
    else if (title.indexOf("【多选题】") != -1) return 5;
    else return -1;
  }
  ...
  async queryAnswers(detail, bypass) {
    //第二参数bypass代表,如果查不到的答案的题目数量超过bypass,则直接返回空
    let set = detail.set;
    ...
    if (bypasscount > bypass)
      return { answers: undefined, count: set.length - bypasscount };
    return { answers, count: set.length - bypasscount };
  }
  async submitTest(detail, answers, params, submit) {
    //提交测验
    let pyFlag = "";
    if (!submit) pyFlag = "1";
    ...
    return this.user.net.post(
      `work/addStudentWorkNew?_classId=${params.classId}&courseid=${params.courseId}&token=${params.enc_work}&totalQuestionNum=${params.totalQuestionNum}&ua=pc&formType=post&saveStatus=1&version=1`,
      built
    );
  }
}
